...
public class ProductDao {
	...
	public ArrayList<HashMap<String, Object>> getAllProductList(String searchCategory, String searchName, String order, int startRow, int rowPerPage) throws Exception{
		...
		String sql = "SELECT distinct p.product_no, p.category_name, p.product_name, p.product_price, p.product_status, p.product_sale_cnt, pimg.product_ori_filename, pimg.product_save_filename, pimg.product_filetype, d.discount_rate, ifnull(ROUND((1 - d.discount_rate) * p.product_price), p.product_price) discount_price FROM product p LEFT OUTER join product_img pimg ON p.product_no = pimg.product_no LEFT OUTER JOIN  discount d ON p.product_no = d.product_no " + searchQuery + " " + orderQuery + " limit ?, ?";
